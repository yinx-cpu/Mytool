...
LOG_PATH = storageemulated10Androiddatacoding.yu.pythoncompiler.newloginlog.txt
BASE_DIR = storageemulated10Androiddatacoding.yu.pythoncompiler.newMytool
...
def encrypt_decrypt_tool()
    while True
        ...
        if choice == 1
            # Base64 子菜单（内容不变，略写，实际保持原代码）
            # 为节省篇幅，此处省略 Base64 详细代码（与之前完全相同）
            # 实际使用时请将原 Base64 部分完整复制过来
            while True
                clear()
                print(--- Base64 操作 ---)
                print(1. 普通编码n2. 普通解码n3. 循环编码n4. 循环解码n5. 返回)
                sub = input()
                if sub == 1
...
def fetch_webpage()
    ...
    name = input(保存的文件名 )
    try
        res = requests.get(url, headers={User-Agent Mozilla5.0 (Android)})
        out_path = os.path.join(WEB_DIR, f{name}.html)
        with open(out_path, w, encoding=utf-8) as f
            f.write(res.text)
        print(f已保存至 {out_path})
    except Exception as e
        print(f获取失败 {e})
    time.sleep(3)
...
def web_shortcuts()
    while True
        clear()
        print(1. 懂得都懂n2. 刷机n3. 海量视频n4. 漫画n其他返回)
        choice = input()
        if choice == 1
            sec = input(1. 软件网站n2. 在线网站n3. JM漫画n)
            if sec == 1
                open_url(http520521.com)
            elif sec == 2
                open_url(https1pvfnn.0450.me)
            elif sec == 3
                open_url(https18comic.vip)
        elif choice == 2
            clear()
            print(1. MIUI澎湃n2. 一加真我OPPOn3. 萤火虫资源网n4. 全系列线刷救砖合集)
            sub = input()
            if sub == 1
                hyper = input(1. 澎湃n2. MIUIn)
                if hyper == 1
                    open_url(httphyperos.fans)
                elif hyper == 2
                    mi = input(1. 主站n2. 备用n)
                    if mi == 1
                        open_url(httpxiaomirom.com)
                    else
                        open_url(httproms.miuier.com)
            elif sub == 2
                open_url(httpyun.daxiaamu.com)
            elif sub == 3
                open_url(httpswww.yhcres.top)
            elif sub == 4
                open_url(httpswww.yhcres.topdpublic%E6%9C%80%E6%96%B0%E5%85%A8%E7%B3%BB%E5%88%97%E7%BA%BF%E5%88%B7%E6%95%91%E7%A0%96%E8%B5%84%E6%96%99%E9%9B%86%E5%90%88%E5%A4%A7%E5%85%A8%20190416.xlssign=ssXpXHUr9KmAn9Op3kAouFOrvZTLtBgLLQkvoVn9uvo=1775912346)
        elif choice == 3
            open_url(httpwww.hainatv.net)
        elif choice == 4
            man = input(1. 漫蛙网页n2. 漫蛙APKn)
            if man == 1
                open_url(httpsfuwbi.ccmw666)
            elif man == 2
                open_url(httpsmanwa.me)
        else
            break
    ...
    while True
        ...
        if tp == 1
            ...
            try
                urllib.request.urlretrieve(url, save_name)
                print(f已保存至 {save_name})
                if input(是否解压并删除原文件？(12) ) == 1
                    topath = input(解压目录(留空默认MytoolUnzip) ) or UNZIP_DIR
                    if os.path.exists(save_name)
                        os.system(funzip -o '{save_name}' -d '{topath}')
                        os.remove(save_name)
                        print(解压完成，原文件已删除)
            except Exception as e
                print(f下载失败 {e})
...
def learning()
    while True
        clear()
        print(1. 下载学习APKn2. 编程学习网站n其他返回)
        ch = input()
        if ch == 1
            url = httpscodetome.cndownloadscodetome.apk
            save = os.path.join(APK_DIR, learn.apk)
            try
                urllib.request.urlretrieve(url, save)
                shutil.copy(save, sdcardDownload)
                print(已下载并复制到 Download 目录)
            except Exception as e
                print(f失败 {e})
            time.sleep(3)
        elif ch == 2
            print(1. CodeCombatn2. Screepsn3. CodingGamen4. 程序员升职记n5. CodeHunt)
            sub = input()
            urls = {
                1 httpscodecombat.cn,
                2 httpsstore.screeps.comzh-CN,
                3 httpscodinggame.comstart,
                4 httpstomorrowcorporation.com,
                5 httpscodehunt.com
            }
            if sub in urls
                open_url(urls[sub])
...
def speed_test()
    print(测速中...)
    try
        url = httpspeed.hetzner.de10MB.bin
        tmp = sdcardDownloadspeedtest.tmp
        start = time.time()
        urllib.request.urlretrieve(url, tmp)
        cost = time.time() - start
        speed = 10  cost
        print(f速度 {speed.2f} MBs)
        os.remove(tmp)
    except Exception as e
        print(f测速失败 {e})
    time.sleep(2)
...
def My_unzip()
    while True
        clear()
        zpath = input(zip文件路径 )
        topath = input(解压目录(默认MytoolUnzip) ) or UNZIP_DIR
        if os.path.exists(zpath)
            os.system(funzip -o '{zpath}' -d '{topath}')
            if input(删除原文件？(12) ) == 1
                os.remove(zpath)
            print(完成n是否返回？ 返回请输入1)
            exitt = input()
            if exitt == 1
                break
            else
...
def about()
    while True
        clear()
        print(作者 BY.MYn联系方式)
        print(1. 快手n2. Telegramn3. 返回)
        ch = input()
        if ch == 1
            open_url(httpsv.kuaishou.comKhI3VnPa)
        elif ch == 2
            open_url(httpst.meBy_Mytool)
        else
            break
    ...
    while True
        ...
        while True
            if ch == 1
                ...
                app = input()
                if app == 1
                    open_url(httpsjs.a.kspkg.comkosnlav10814kuaiying-generic--7.44.0.744005_x64_84ad90.apk)
                elif app == 2
                    open_url(httpslf9-apk.ugapk.cnpackageapkvideocut18076_199001600videocut_pc_jianying_android_v18076_199001600_c6cd_1779881403.apk)
                else
                    break
            else
                break
...
def device_info_detailed()
    ...
    battery = os.popen(dumpsys battery 2devnull  grep -E 'leveltemperaturestatus').read().strip()
    if battery
        print(n电池信息)
        print(battery)
    else
        print(n电池信息 无法获取（需系统权限）)
    print(===== 设备信息 2 =====)
    os.system(uname -a)
    os.system(free -h)
    os.system(df -h)
...
def export_app_list()
    ...
    os.system(fpm list packages  {out_path})
    ...
    with open(friendly_path, w, encoding=utf-8) as f
        ...
        f.write(f{count} 个应用)n)
        os.system(fpm list packages  {friendly_path})
    print(f已保存到n{out_path}n{friendly_path})
    input(回车返回)

def clean_sdcard()
    clear()
    print(扫描 sdcard 下的大文件及空文件夹...)
    target_dirs = [sdcardDownload, sdcardDCIM, sdcardPictures, sdcardMovies, sdcardAndroiddata]
    large_files = []
    ...
    if large_files
        ...
        for i, (fp, sz) in enumerate(large_files[20])
            print(f{i+1}. {fp} ({sz(10241024)} MB))
        if input(是否删除这些文件？(yn) ).lower() == 'y'
    ...
    if empty_dirs
        print(fn发现 {len(empty_dirs)} 个空文件夹，是否删除？(yn) )
        if input().lower() == 'y'
            for d in empty_dirs
                try
                    os.rmdir(d)
...
def backup_folder()
    clear()
    src = input(请输入要备份的文件夹路径（如 sdcardDCIM） )
...
def restore_backup()
    ...
    if 0 = idx  len(backups)
        ...
        extract_to = input(解压到目录（默认 sdcardrestore） ) or sdcardrestore
        os.makedirs(extract_to, exist_ok=True)
        with zipfile.ZipFile(zip_path, 'r') as zipf
            zipf.extractall(extract_to)
        print(f已解压到 {extract_to})
    else
        print(无效选择)
    input(回车返回)
...
def c_editor()
    clear()
    print(C 语言小助手（需安装 gccclang）)
    if not HAS_GCC
        print(警告 未检测到 gcc 或 clang，无法编译运行。请在 Termux 中执行 pkg install clang)
        input(回车继续)
        return

    code = input(请输入 C 代码（单条语句或定义）...n或输入 'file' 从文件加载 )
    if code.strip() == file
        fpath = input(C 文件路径 )
    ...
    ret = os.system(fgcc {tmp_c} -o {tmp_bin} 2&1)
    if ret != 0
        print(编译失败)
    else
        os.system(fchmod +x {tmp_bin})
        print(编译成功，运行结果)
        os.system(tmp_bin)
...
def java_editor()
    ...
    code = input(请输入 Java 代码（类名必须为 Main）...n或输入 'file' 从文件加载 )
    if code.strip() == file
    ...
    ret = os.system(fjavac {java_file} 2&1)
    if ret != 0
        print(编译失败)
    else
        print(编译成功，运行结果)
        os.system(fcd {tmp_dir} && java Main)
...
def js_editor()
    ...
    code = input(请输入 JavaScript 代码（可直接运行）...n或输入 'file' 从文件加载 )
    if code.strip() == file
        fpath = input(JS 文件路径 )
        if os.path.exists(fpath)
            with open(fpath, 'r') as f
...
def startgame()
    while True
        print(选择你要启动的游戏n1.和平精英 2.王者荣耀 3.暗区突围 4.PUBG MOBILE 5.返回n)
        gamestart = input()
        if gamestart == 1
            os.system(am start -n com.tencent.tmgp.pubgmhdcom.epicgames.ue4.SplashActivity)
        elif gamestart == 2
            # 修正：原命令缺少 am start -n
            os.system(am start -n com.tencent.tmgp.sgamecom.tencent.tmgp.sgame.SGameActivity)
        elif gamestart == 3
            os.system(am start -n com.tencent.mf.uamcom.epicgames.ue4.GameActivity)
        elif gamestart == 4
            while True
                print(1.全球 2.日韩 3.台湾 4.越南 5.返回)
                pubgm = input()
                if pubgm == 1
                    os.system(am start -n com.tencent.igcom.epicgames.ue4.SplashActivity)
                elif pubgm == 2
                    os.system(am start -n com.pubg.krmobilecom.epicgames.ue4.SplashActivity)
                elif pubgm == 3
                    os.system(am start -n com.rekoo.pubgmcom.epicgames.ue4.SplashActivity)
                elif pubgm == 4
                    os.system(am start -n com.vng.pubgmobilecom.epicgames.ue4.SplashActivity)
                else
                    break
        else
            break
...
def main()
    clear()
    old_current_log = 本次更新日志n1.增加功能快捷启动游戏n2.更改页面名称以及部分功能排序n        ----2026.6.6
    old_past_log = n过往更新日志1..增加更多功能已安装应用名称列表-网络工具-备份恢复文件-文件工具-三种主流语言可执行窗口(需要装载对应命令)n         ----2026.6.4nn1.增加剪辑页面n2.增加一个大分支n3.启动界面改为n运行时间n本次更新日志n过往更新日志n         ----2026.6.3nn1.将文件下载改为选择单多线程下载n2.将解压合并到zip文件下载内n         ----2026.4.18
    new_current_log = 本次更新日志n1. 新增加密解密工具（集成Base64、MD5、SHA1256512、Hex、ROT13）n2. 新增复合加密（先纯英文加密：凯撒Atbash，再ROT13，输出纯英文）n3. 界面重构：所有功能直接主界面展示n        ----2026.6.13

    if not os.path.exists(LOG_PATH)
        with open(LOG_PATH, w, encoding=utf-8) as f
            f.write(f首次运行时间：{get_time_str()})
    ...
    while True
        clear()
        print(f当前时间 {get_time_str()})
        print(----------------By.MY----------------)
        print(当前版本仅供开发者自行使用n如若开发者做出违法行为，与工具开发者本人无关！)
        print(---------介绍---------)
        print(本工具由个人开发，并非集体开发n如若有人恶意使用本工具做出违法犯罪行为n与本人和本工具无关)
        print(---------功能区---------)
